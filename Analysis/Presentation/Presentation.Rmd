...
- Pool short-term liabilities to fund long-term assets.

- Perform other important activities (payment process).

- Central role to monetary policy transmission [@bernanke1995inside].

- unstable, prone to runs [@diamond1983bank].

## Capital Requirements


- Rationale: Compensate for public guarantees and externalities that result in banks having less capital than socially desirable. 

- However, there is no consensus on appropriate level and implementation [@diamond2000theory; @diamond2001liquidity; @admati2014bankers].

- Hence, regulation on capital requirements have changed substantially since first international standard.

- Arguably, risk sensitivity is at the center of the changes [@danielsson2001academic; @basel2004international; @EBA2019policy]


## Capital Requirements and Risk Sensitivity

- Basel I (July 1988): Sets fixed risk-weights for assets within same class. 

\begin{equation*}
\begin{aligned}
\label{eq5}
\text{Capital Ratio} = \frac{K}{\sum_j\alpha_j\sum_i  x_{i,j}} \geq k
\end{aligned}
\end{equation*}


$K$: equity capital \newline
$x_i$: asset $i$ from class $j \in \{\text{Corporate, Retail, Sovereign, etc...} \}$ \newline
$\alpha_j$: weight for assets in class $j$ \newline
$k$: capital ratio requirement \newline

\pause

- Problem: simple rule may incentivize banks to hold more risky portfolios and increase probability of failure [@koehn1980regulation]


## Capital Requirements and Risk sensitivity

Basel II (2004): Introduces the possibility to use internal-based rating (IRB) models to calculate risk-weights 

\begin{equation*}
\begin{aligned}
\label{eq5}
\text{Capital Ratio} = \frac{K}{\sum_j\sum_i \alpha_{i,j}  x_{i,j}} \geq k
\end{aligned}
\end{equation*} 

$\alpha_{i,j}$: weight for assets $i$, estimated using banks own risk models. 

\pause

Rationale: 

- Riskier banks will hold more capital [@kim1988risk].
- Alignment of risk estimates to actual risk [@barakova2014banks].  

\pause

Problems: 

- Enhances procyclical nature of lending [@behn2016procyclical].
- Allow differences for reasons other than risk [@mariathasan2014manipulation; @behn2016limits].

## Credit Risk Distribution and Capital Savings 

- This paper contributes to the understanding of risk-sensitive capital requirements ($\alpha_{i,j}$).

- I identify a novel effect stemming from the design of the IRB framework: banks save capital requirements because mapping from risk (PD) is concave.

- I collect data and propose a measure of this effect.

- Key findings:
  - Reduces the procyclicality of RWA by 11.2 percentage points. 
  - Explains 27% of RW variation across banks. 

## Concavity of Capital Requirements
...
p + geom_line(alpha = 1) +
  ...
  scale_x_continuous("",breaks=c(PD.h+0.03),labels=c("PD"),expand = c(0, 0))+
  scale_y_continuous("",breaks=c(K.h),labels=c("RWA"),expand = c(0, 0)) 
...
\begin{aligned}
\label{eq5}
\text{RWA} & = \text{RWA}_{SA}+\text{RWA}_{IRB} \\
\pause
& = \text{RWA}_{SA}+\text{RWA}^{c}-\text{RWA}^{\$}\\
\pause
& = \text{RWA}_{SA}+\sum_{j=1}^P\text{RWA}^{c}_j-\text{RWA}^{\$}\\
\end{aligned}
\end{equation*}
...
\begin{aligned}
\label{eq5}
\frac{\text{RWA}}{\text{EAD}}&= \frac{\text{RWA}_{SA}}{\text{EAD}_{SA}}\frac{\text{EAD}_{SA}}{\text{EAD}}+\frac{\text{EAD}_{IRB}}{\text{EAD}}\sum_{j=1}^P\frac{\text{EAD}_{IRB,j}}{\text{EAD}_{IRB}}\frac{\text{RWA}^{c}_j}{\text{EAD}_{IRB}}+\frac{\text{RWA}^{\$}}{\text{EAD}_{IRB}}\frac{\text{EAD}_{IRB}}{\text{EAD}}\\
\pause
\text{RW} &= \text{RW}_{SA}q_{SA}+(1-q_{SA})\sum_{j=1}^P q_{j}\text{RW}^{c}_j +(1-q_{SA})\text{RW}^{s}\\
\end{aligned}
\end{equation*}
...
\begin{aligned}
\label{eq5}
\text{RW}-\text{RW}^B =& \text{RW}_{SA}q_{SA}+(1-q_{SA})\sum_{j=1}^P q_{j}\text{RW}^{c}_j +(1-q_{SA})\text{RW}^{s}\\
&- \text{RW}_{SA}^Bq_{SA}^B+(1-q_{SA}^B)\sum_{j=1}^P q_{j}^B\text{RW}^{c,B}_j +(1-q_{SA})\text{RW}^{s,B}
\end{aligned}
\end{equation*}
...
\begin{aligned}
\label{eq5}
\Delta \text{RW} & = \underbrace{q^{B}_{SA} \Delta \text{RW}_{SA}}_\text{SA mix}+\underbrace{(1-q^{B}_{SA})\sum_{j=1}^{P} q^{B}_{j}\Delta \text{RW}^{c}_j}_\text{Mean IRB Risk}\\
&\\
&+ \underbrace{(1-q^{B}_{SA})\sum_{j=1}^{P} \text{RW}_{j}^{c} \Delta q^{j}}_\text{IRB mix}  + \underbrace{(\text{RW}_{SA}-\text{RW}^{c})\Delta q_{SA}}_\text{Roll-out}\\
&\\
...
stargazer(reg.list,
          ...
          table.layout = "n=c=d#-t-as-",
          notes = "\\parbox[t]{5cm}{The table shows estimates for the following model: \\newline \\begin{equation*}\\text{RWA}_{i,j,t}^{s} = \\beta\\times X_{i,t}+\\alpha_{i}+\\alpha_{t}+\\varepsilon_{i,t}. \\end{equation*}}",
          #omit.table.layout = "n",
          dep.var.labels   = dep.text,
          covariate.labels = ind.text,
          se               = se.list,
...
- Capital Savings due to the concavity of  are economically relevant.

- Countercyclical nature may offer another policy tool to reduce procyclicality of capital regulation.

- Potential risk: banks can use same effect to increase average risk while keeping capital requirements constant.
