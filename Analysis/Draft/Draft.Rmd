...
pre.treat.out <- bank.data %>%
  ...
  select(bvdid, log.RWA.total, log.RWA.other, log.RWA.credit, log.RWA.IRB, log.RWA.SA, log.RWA.hat, log.RWA.s,
         log.K, log.tier, totalcapitalratio, tier1ratio, CAR.dif, tier.dif) %>%
  group_by(bvdid) %>%
...
DiD.data <- bank.data %>%
  ...
  mutate(Shortfall.mn   = log(1+Shortfall.mn), 
         Shortfall.mn.2 = ifelse(EBAbank == 1,Shortfall.mn, 0),
         Shortfall.RW.2 = ifelse(EBAbank == 1,Shortfall.RW, 0),
...
```
```{r, include=FALSE}
ggplot(DiD.data, aes(x= EBAbank, y =  out.tier.ratio)) + geom_point() +geom_smooth(method = "lm")
```
...
reg.4 <- felm(out.RWA.total ~ Shortfall.mn.2 
                 | 0 |0|0,
                  data = DiD.data)
