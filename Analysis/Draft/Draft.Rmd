...
Many studies have questioned the reliability of banks' calculations of risk-weighted assets (RWA) for prudential purpose. The significant divergences found across banks are taken as indicating excessive subjectivity in the current rules governning banks' risk measurement and capital requirement calculations. Importantly, this subjectivity is unintended since differences in RWA driven by it do not reflect differences in risk. This paper shows that the dispersion _within_ banks' portfolio credit risk, more specifically the probability of default (PD) parameter, explains substantially the variation in RWA _across_ banks.  

paragraph connecting to previous literature
Basel II introduced a concave mapping from probabilities of default to capital requiremets. As a consequence, banks can reduce requirements or increase risk-taking by widening their portfolio credit risk distribution.

To calculate equation X, or any other measure of capital savings arising from the concavity of the regulatory formula,  one needs data on individual, or group of assets' regulatory risk parameters. However, such granularity is not available in the common commercial balance sheet datasets, and, although very detailed, datasets from credit registers are restricted to one country and are not harmonized. Therefore, to analyze the drivers and implications of PD dispersion, I collected information on portfolio distribution from banks' individual Pillar-3 reports.\footnote{More specifically, I collected the information contained in tables that follow the CR5, CR6, CR10, CCR3, and CCR4 templates from the BCBS revised version of the Pillar 3 framework, or equivalent.} The sample consists of 50 large banking groups, from 10 countries, that have adopted the IRB approach. For each bank, I collected group consolidated information from the year the IRB approach was approved, which is early as 2007, until 2018. The dataset distinguishes between wholesale, retail and equity portfolios and, in most cases, between their sub-categories.\footnote{The sub-categories for wholesale are, corporate, banks, sovereign, and specialized lending, for retail, real estate, qualifying revolving credit, and other retail, and for equity, portfolios under the PD/LGD approach, and under the simple approach.} This level of portfolio breakdown is important because the regulatory formula varies across categories. It also allows me to control for variation in capital requirements due to business mix (sovereign portfolios require less capital than corporate) that could  be otherwise captured by my capital savings measure. Information on the share of portfolio under the standardized approach is also gathered, which allow me to control for the roll-out effect, i.e., strategical reduction in capital requirements by moving specific portfolios from the standardized to the IRB approach. Finally        
...
plot2 <- bank.data %>% mutate(tier1ratio= tier1ratio/100) %>% filter(year>2001)%>%
  mutate(time = lubridate::ymd(year, truncated = 2L))%>%
ggplot(aes(x=time)) +
...
plot3 <- bank.data %>% mutate(ROE = ROE/100) %>% filter(year>2001)%>%
  mutate(time = lubridate::ymd(year, truncated = 2L))%>%
ggplot(aes(x=time)) +
...
```{r, fig.height = 4, fig.width = 5, results="hide",fig.cap= fig1_cap,out.width = "1\\textwidth"}
time.decomposition %>% 
  select(time,  SA.gain, IRB.gain, mix.gain, dispersion.gain, rollout.gain) %>%
...
banks. We restrict the analysis to banks for which we are able to collect information from Pillar-III reports. 
```{r, fig.height = 4, fig.width = 5, results="hide",fig.cap= fig1_cap,out.width = "1\\textwidth"}
year.decomposition %>% 
  select(year,  SA.gain, IRB.gain, mix.gain, dispersion.gain, rollout.gain) %>%
  gather(condition, value, contains(".gain"), factor_key=TRUE)%>%
  mutate(condition = relevel(condition,"dispersion.gain")) %>%
...
cross.section.decomposition %>% 
  ...
  select(name, Country, RW,order.s, SA.gain, IRB.gain, mix.gain, dispersion.gain, rollout.gain) %>%
  gather(condition, value, contains(".gain"), factor_key=TRUE)%>%
  mutate(condition = relevel(condition,"dispersion.gain")) %>%
...
```{r relation, fig.height = 4, fig.width = 5, results="hide",fig.cap= fig1_cap,out.width = "1\\textwidth"}
 bank.data %>% 
  select(name,RW.s, tier1ratio, tier.hat) %>%
  group_by(name)%>%
...
```{r relation, fig.height = 4, fig.width = 5, results="hide",fig.cap= fig1_cap,out.width = "1\\textwidth"}
bank.data %>% 
  select(name,RW.s, totalcapitalratio, CAR.hat) %>%
  group_by(name)%>%
 summarise_if(is.numeric,funs(mean),na.rm=TRUE)%>%
