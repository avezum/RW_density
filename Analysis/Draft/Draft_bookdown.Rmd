...
The results from Tables \ref{tab:main} and \ref{tab:alt} raises the question of why it appears that only *Capital savings* from corporate portfolios are impacted during the business cycle. In this section I argue that portfolio reallocation during the business cycle explains part of *Capital savings* cyclicality. Ideally, for this purpose, I would track individual assets during the cycle and compare their PDs evolution to the PDs of new loans.  Unfortunately, I can only observe assets aggregated by PD bands which does not allow me to track individual assets. As an alternative approach, I analyze the relationship of PD distribution moments with GDP to understand asset allocation during the business cycle. 

According to the theoretical framework presented in section 4.1, an increase in average PD reduces the amount of *Capital savings* if other moments are kept constant. This relationship is illustrated in panel A of Figure \ref{fig:pdmapcases} and empirically tested in Table \ref{tab:moments_contribution}. In all columns of this table average PD is negatively associated with *Capital savings*. Table \ref{tab:moments_contribution} also shows that, in line with the theoretical framework, an increase in PD variance is positively associated with *Capital savings* for all portfolios. In sum, the estimated coefficients of the average PD and the PD variance provide strong empirical support to the theoretical framework presented in section 4.1. At the same time, however, I cannot refute the hypothesis that portfolio reallocation does not impact *Capital savings* because the estimated effect of these two moments on *Capital savings* are in line with a scenario without any portfolio reallocation.  

Extending the analysis to the skewness and kurtosis of the PD distribution proves to be very helpful. Consider a  



Panel A in table \ref{tab:moments_cyclicality} reports how average PD moves during the business cycle. As expected, in most of the regressions, changes in average PD are negatively associated with GDP growth. This association is the known driver of the procyclical character of the current model-based capital regulation and its interpretation is fairly mechanical. During recessions average credit quality deteriorates, or in other words, average PD increases. The only exception to the rule is the sovereign portfolio. Column 5 of panel A shows that the coefficient of GDP growth on changes on average PD of sovereign portfolios is positive, although without strong evidence that it is not different from zero. One possible reason for this result is that, during economic downturns, banks may search for safe assets mostly in the form of safe haven government bonds. According to column 5 of panel A, this reallocation of portfolio appears to be sufficient to counterbalance the deteriorations of the prior portfolio. It also helps explain the insignificant results in column 5 of Tables \ref{tab:main} and \ref{tab:alt}. 

As explained, if no other moment is affected during the business cycle we should observe procyclical *Capital savings*. However, it was shown in     

Common  
I further analyse the relationship of the PD distribution moments with GDP and find that during economic expansions: average, variance and kurtosis decrease while the skewness increases. This evidence is in line with the idea that the upper-tail (riskier borrowers) of the PD distribution is disproportionately affected by the business cycle.
...
# Run the models
for(i in dep.vars){
formula.text <- formula(paste0("log.RWA.r", i, "_gr ~ mean + var + skew + kurt  | year + bvdid | 0 | Country "))
 data = rename(bank.data.win,
               mean    = paste0("mean.PD"  , i, "_gr"),
               var     = paste0("var.PD"   , i, "_gr"),
               skew    = paste0("skew.PD"  , i, "_gr"),
               kurt    = paste0("kurt.PD"  , i, "_gr"))%>%
