...
author:
- affiliation: CentER and EBC, Tilburg University; l.avezum@uvt.nl; +31134664531;  Warandelaan
    2, 5037 AB Tilburg, NL, The Netherlands
  name: Lucas Avezum
date: '\emph{\today}'
output:
...
fontsize: 11pt
geometry: margin= 1in
header-includes: \usepackage{rotating, graphicx, dcolumn, longtable, float}
...
abstract: The Basel II accord introduced a concave mapping from probabilities of default
  ...
  capital savings. I document that these capital savings are important sources of
  risk-weight variability across banks. I further investigate the relationship of
  capital savings with the business cycle and find it to be countercyclical. The results
  suggest that a recalibration of the current IRB framework can significantly reduce
...
The follow-up to the 2008 financial crisis offer an plausible explanation for the dynamics of *Capital savings* in time, which is illustrated in Figure \label{fig:timevariables}. Increased supervisory pressure to increase capital ratios (top plot), especially after the Basell III, led banks to deleverage and reduce risk-taking. [^1] The shift for safer assets not only reduced the average PD on banks' portfolios (middle plot) but also compressed their PD distribution (bottom plot). This distribution compression leads to a reduction in *Capital savings*. This explanation is in line with the hypothesis that *Capital savings* are countercyclical. Efforts to increase capital ratio by risk reduction are increasingly more difficult.         

[^1]: Although supervisors favored raising additional capital as means to achieve the required capital ratios [@EBA2011recommendation], banks respond to capital requirements mostly by reducing RWA [@gropp2018banks].
...
In Table \ref{tab1}, I regress the change in the logarithm of *Capital savings* on the change in the logatithm of real GDP per capita. In all regressions the measure of *Capital savings* follows equation \ref{eq4}. The difference between regressions in the table is the portfolio used to calculate the *Capital savings*. The real GDP per capita used  comes from the country where the bank has its headquarter. All regressions include bank and year fixed effects and standard errors clustered at the country level.        

Column 1 shows that total *Capital savings* are countercyclical as the growth rate of tota l*Capital savings* is negatively associated to the GDP growth rate. According to regression 1 an increase of 1.6 percentage points in GDP growth rate (one standard deviation) decreases total *Capital savings* growth rate by 0.7 percentage points on average. The effect is economically sizeable representing 14% of total *Capital savings* standard deviation in the sample. 

The remaining columns of Table \ref{tab1} suggest that the countercyclical relationship between GDP and *Capital savings* is mostly driven by wholesale portfolios, especially corporate portfolios. The estimated coefficient in Column 2 is negative suggesting that *Capital savings* generated by retail portfolios are also countercyclical. However, this coefficient is not statistically significant. On the other hand, the coefficient of GDP growth rate on *Capital savings* from wholesale portfolios is statistically and economically significant. According to the regression 3 one standard deviation increase in GDP growth rate decreases *Capital savings* from wholesale portfolios growth rate by 0.8 percentage points on average, which is 16% of the *Capital savings* from wholesale portfolios standard deviation. Columns 3 to 6 indicate that within the wholesale portfolio most of the countercyclical variation appears to come from corporate portfolios. Only in regression 4 the coefficient of GDP growth rate is negative and statistically significant.  

## Robustness to Alternative Measures

A concern to the previous analysis is the possibility of that I could be overestimating the average impact of GDP growth rate on *Capital savings*. First, Because the measure used in Table \ref{tab1} is dimentionless, i.e., it is a measure of intensity of *Capital savings*, if the portfolio of small banks, in terms of asset, are more impacted during the business cycle than the portfolio of large banks then the cofficients in Table \ref{tab1} overestimate the impact to the average bank. To account for differences in *Capital savings* size, in Table \ref{tab2} the measure of *Capital savings* follows equation \ref{eq3}, which is a measure of *Capital savings* in monetary units. 

The results shown in Table \ref{tab2} reinforce the evidence that the coutercyclicality of *Capital savings* is mostly driven by the corporate portfolio. 

Second, because both measures in Tables \ref{tab1} and \ref{tab2} are relative to the IRB portfolio, if the portfolio of banks with lower share of their portfolio under the IRB approach are more impacted during the business cycle than the portfolio of high IRB share banks then the cofficients in Table \ref{tab1} overestimate the impact to the average bank. The results shown in Table \ref{tab2} reinforce the evidence that the coutercyclicality of *Capital savings* is mostly driven by the corporate portfolio.

Lastly,  In Table \ref{tab3} I use the Gini coefficient as an alternative measure of the shape component of the PD distribution. Again, the results shown in Table \ref{tab3} suppport the evidence from the last three tables: *Capital savings* are countercyclical and mostly driven by the effect of GDP growth rate on *Capital savings* from corporate portfolios.   


## Capital Savings and Portfolio Realocation

Ideally we want to fix the porfolio at some point in time and compare its and new loans PD distribution. Unfortunately, I cannot track individual assets of a portfolio. In this section I compare moments of the PD distribution between good and bad times without disantangling the direct effect of the business cycle on the distribution from the realocation effect. 

The results from Tables \ref{tab1} to XX raises the questions of why it appears that only corporate portfolios are impacted during the business cycle. 
...
\multicolumn{6}{c}{\parbox[t]{16cm}{The table shows estimates for the following model: \newline \begin{equation*}\Delta \text{Y}_{i,t}^{j} = \beta \Delta\text{Log GDP}_{i,t}+\alpha_{i}+\alpha_{t}+\varepsilon_{i,t}. \end{equation*} \newline 
The dependent variable is the change in the logarithm of \textit{Capital savings} for portfolio $j$. In Panel A the measure of \textit{Capital savings} follows equation \ref{eq5}. In Panel B the measure of \textit{Capital savings} follows equation \ref{eq6}. In Panel C the Gini coefficient is used as alternative measure of the shape of the PD distribution. $\Delta\text{Log GDP}_{i,t}$ is the real GDP per capita growth rate. All regressions include bank and year fixed effects. Robust standard errors adjusted for clustering at the country level are reported in parentheses. *, **, and *** indicate statistical significance at the 10\%, 5\%, and 1\% level respectively.}} \\
[-1.8ex] \\
\hline \\
\end{tabular}
...
stargazer(reg.list,
          title = "Alternative measure: PD distribution Gini Coefficient",
          table.layout = "=c=d#-t-as-",
          dep.var.labels   = dep.text,
          covariate.labels = ind.text,
          #se               = se.list,
...
stargazer(reg.list,
          ...
          table.layout = "n=c=d#-t-as-",
          notes = "\\parbox[t]{14cm}{The table examines the relationship between the business cycle and other components of RWA. The dependent variable is the change in the logarithm of risk-weighted assets for portfolios under the standardize approach in column 1, the change in the logarithm of risk-weights for portfolios under the standardize approach in column 2, the change in the logarithm of the ratio of exposure under the standardize approach to total exposure in column 3, the change in the logarithm of the share of the IRB portfolio as wholesale exposure in column 4, and the change in the logarithm of the share of the IRB portfolio as retail exposure in column 5. $\\Delta\\text{Log GDP}_{i,t}$ is the real GDP per capita growth rate. All regressions include bank and year fixed effects. Robust standard errors adjusted for clustering at the country level are reported in parentheses. *, **, and *** indicate statistical significance at the 10\\%, 5\\%, and 1\\% level respectively.}",
...
# Variables in the model 
dep.vars <- c("mean.PD_gr", "var.PD_gr", "skew.PD_gr", "kurt.PD_gr", "log.RWA.r_gr")
aux.1 <- c("$\\Delta \\text{Mean(PD}_{i,t}$)", "$\\Delta \\text{Variance(PD}_{i,t}$)", "$\\Delta \\text{Skewness(PD}_{i,t}$)", "$\\Delta \\text{Kurtosis(PD}_{i,t}$)")
aux.2 <- c( "$\\Delta\\text{Log RWA}_{i,t}^{s}$")
dep.text <- c(aux.1, aux.2)
ind.vars <- c("mean.PD_gr", "var.PD_gr", "skew.PD_gr", "kurt.PD_gr")
ind.vars  <- paste(ind.vars, collapse=" + ")
ind.vars <- c(rep("log.gdp_gr", 4), ind.vars)
ind.text <- c("$\\Delta\\text{Log GDP}_{i,t}$", aux.1)
...
# Model formulas
formula.text <- paste0(paste0(dep.vars)," ~ ", paste0(ind.vars)," | year + bvdid | 0 | Country ")
formulas <- lapply(formula.text, formula)
...
dep.text <- c("$\\Delta\\text{Log capital ratio}_{i,t}$","$\\Delta\\text{Log capital}_{i,t}$", "$\\Delta\\text{Log RWA}_{i,t}$", "$\\Delta\\text{Log RWA}_{i,t}^{c}$", "$\\Delta\\text{Log RWA}_{i,t}^{s}$")
# Independent variables
ind.vars <- paste0("log.gdp_gr")
ind.text <- c("$\\Delta\\text{Log GDP}_{i,t-1}$")
...
# Run the models
reg.list <- lapply(formulas, function(x) {
  felm(x, data = filter(bank.data.win, RWA.s>0 & !is.na(log.CAR_gr)))
})
...
\footnotesize
\begin{tabular}{@{\extracolsep{1pt}}lccccc}
\multicolumn{6}{c}{\parbox[t]{12cm}{The table shows summary statistics for banks that have introduced the IRB approach between 2007 and 2018. All variables are winsorized at 5\%}} \\
\end{tabular}

```{r summary_components, fig.height = 10, fig.width = 7, results="asis",out.width = "1\\textwidth"}
bank.data.win %>% select(
  ...
  log.RWA.r.Banks_gr,
  log.RWA.hat_gr, 
  log.RWA.hat.Retail_gr,
  log.RWA.hat.Wholesale_gr, 
  log.RWA.hat.Corporate_gr, 
  log.RWA.hat.Sovereign_gr, 
  log.RWA.hat.Banks_gr) %>%
  as.data.frame() %>% 
stargazer(header = FALSE, type="latex", single.row = TRUE, column.sep.width = "1pt", font.size = "footnotesize",
          ...
          covariate.labels = 
            c("$\\Delta \\text{Log Capital Ratio}$",
              "$\\Delta \\text{Log Adjusted Capital Ratio}$",
              "\\hspace{0.5cm}$\\Delta \\text{Log capital}$",
              "\\hspace{0.5cm}$\\Delta \\text{Log RWA}$",
              "\\hspace{1.0cm}$\\Delta \\text{Log RWA}^c$",
              "\\hspace{1.5cm}$\\Delta \\text{Log RWA}^c$ Retail",
              "\\hspace{1.5cm}$\\Delta \\text{Log RWA}^c$ Wholesale",
              "\\hspace{2.0cm}$\\Delta \\text{Log RWA}^c$ Corporate",
              "\\hspace{2.0cm}$\\Delta \\text{Log RWA}^c$ Sovereign", 
              "\\hspace{2.0cm}$\\Delta \\text{Log RWA}^c$ Banks",
...
```{r summary_other_measures, fig.height = 10, fig.width = 7, results="asis",out.width = "1\\textwidth"}
bank.data.win %>% select(
  RW.s_gr,
  RW.s.Retail_gr,
  RW.s.Wholesale_gr, 
...
stargazer(header = FALSE, type="latex", single.row = TRUE, column.sep.width = "1pt", font.size = "footnotesize",
          ...
          omit.summary.stat = c("p25","p75"),
          covariate.labels = 
            c("\\hspace{0.0cm}$\\Delta \\text{Log RW}^s$",
              "\\hspace{0.5cm}$\\Delta \\text{Log RW}^s$ Retail",
              "\\hspace{0.5cm}$\\Delta \\text{Log RW}^s$ Wholesale",
              "\\hspace{1.0cm}$\\Delta \\text{Log RW}^s$ Corporate",
              "\\hspace{1.0cm}$\\Delta \\text{Log RW}^s$ Sovereign", 
              "\\hspace{1.0cm}$\\Delta \\text{Log RW}^s$ Banks",
              "\\hspace{0.0cm}$\\Delta \\text{Log RWA}^{\\$}$",
              ...
              "\\hspace{0.0cm}$ \\text{Log RWA}^r$",
              "\\hspace{0.5cm}$ \\text{Log RWA}^r$ Retail",
              "\\hspace{0.5cm}$ \\text{Log RWA}^r$ Wholesale",
              "\\hspace{1.0cm}$ \\text{Log RWA}^r$ Corporate",
              "\\hspace{1.0cm}$ \\text{Log RWA}^r$ Sovereign", 
              "\\hspace{1.0cm}$ \\text{Log RWA}^r$ Banks"),
          notes.label = "",
          notes.align = "c", 
          notes.append = FALSE,
          summary = TRUE,
...
```{r summary_portfolio, fig.height = 10, fig.width = 7, results="asis",out.width = "1\\textwidth"}
bank.data.win %>% select(
  mean.PD_gr, 
  mean.PD.Retail_gr,
...
stargazer(header = FALSE, type="latex", single.row = TRUE, column.sep.width = "1pt", font.size = "footnotesize",
          ...
          covariate.labels = 
            c("$\\Delta$ Log PD Mean",
              ...
              "$\\Delta$ Log PD Variance",
              "$\\Delta$ Log PD Skewness", 
              "$\\Delta$ Log PD Kurtosis"),
          notes.label = "",
          notes.align = "c", 
          notes.append = FALSE,
          digits = 3,
...
```{r summary_control, fig.height = 10, fig.width = 7, results="asis",out.width = "1\\textwidth"}
table <- capture.output({bank.data.win %>% ungroup()%>% select(
  log.gdp_gr,
  log.gdp,
  credit_gdp,
  pillarI_stringency,
...
table <- gsub("lccccc",
              "p{0.4\\textwidth}ccccc", table, fixed = T)
