...
mapping_wholesale <- function(PD,LGD,M,alpha,portfolio){
  ...
  K  <- LGD*(punif(((1-R)^(-0.5))*qunif(PD)+((R/(1-R))^(0.5))*qunif(alpha))-PD)*((1-1.5*b)^-1*(1+(M-2.5)*b))
  RW <- K*12.5
...
mapping_retail <- function(PD,LGD,R,alpha,portfolio){
  K  <- LGD*(pnorm(((1-R)^(-0.5))*qnorm(PD)+((R/(1-R))^(0.5))*qnorm(alpha)))
  RW <- K*12.5
  data <- data.frame(K,RW,PD,LGD,M,R,portfolio)
  return(data)
}

mapping_retail_alt <- function(PD,LGD,R,alpha,portfolio){
  K  <- LGD*(pnorm(((1-R)^(-0.5))*qnorm(PD)+((R/(1-R))^(0.5))*qunif(alpha)))
  RW <- K*12.5
  data <- data.frame(K,RW,PD,LGD,M,R,portfolio)
  return(data)
}
...
data <- mapping_retail(grid,0.45,0.15,0.999,factor("Mortgage"))
data <- rbind(data,mapping_retail_alt(grid,0.45,0.15,0.999,factor("Mortgage_alt")))
...
Concavity of RW formula: highly driven by Normality assumption. 

Findings so far: 

- Fatter-tailed distribution of systematic component lead to less concavity (uniform distribution used, tes with others)
